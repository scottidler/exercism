    ...
    pub fn new(chars: Vec<char>) -> Self {
        match chars.len() {
            ...
            1..=10 => Self {
                count: 0,
                place: 0,
                subs: vec![chars_to_substitions(chars)],
            },
            _ => {
                panic!("Too many characters to permute (max 10)");
            }
        }
    }
    */
    ...
    pub fn with_new_chars(&self, chars: Vec<char>) -> &Self {
        let chars: Vec<char> = chars
            .into_iter()
            .filter(|c| !self.contains_key(*c))
            .collect();
        if !chars.is_empty() {
            //zip chars with numbers
