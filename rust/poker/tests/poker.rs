...
use poker::{Card, Hand, Rank};
//use poker::compare_hands;

#[test]
fn test_hand_highcard() {
    let input = "4S 5S 7H 8D JC";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::HighCard(11));
}

#[test]
fn test_hand_onepair() {
    let input = "4S 5S 7H 4D JC";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::OnePair(4));
}

#[test]
fn test_hand_twopair() {
    let input = "4S 5S 4H 5D JC";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::TwoPair(5, 4));
}

#[test]
fn test_hand_threeofakind() {
    let input = "4S 7S 4H 5D 4C";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::ThreeOfAKind(4));
}

#[test]
fn test_hand_straight() {
    let input = "4S 5S 6H 7D 8C";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::Straight(8));
}

#[test]
fn test_hand_flush() {
    let input = "2S 5S 6S 7S 8S";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::Flush(8));
}

#[test]
fn test_hand_fullhouse() {
    let input = "4S 4H 4D 5S 5C";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::FullHouse(4, 5));
}

#[test]
fn test_hand_fourofakind() {
    let input = "4S 4H 4D 4C 5C";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::FourOfAKind(4));
}

#[test]
fn test_hand_straightflush() {
    let input = "4S 5S 6S 7S 8S";
    let hand = Hand::new(input);
    assert_eq!(hand.rank, Rank::StraightFlush(8));
}

#[test]
#[cfg(feature = "imp five of kind")]
fn test_hand_fiveofakind() {
    let input = "4S 4H 4D 4C 4C";
    let hand = Hand::new(input);
